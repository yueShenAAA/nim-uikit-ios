...
@objcMembers
open class ChatViewController: NEBaseViewController, UINavigationControllerDelegate,ChatViewModelDelegate, NIMMediaManagerDelegate,
                               MessageOperationViewDelegate, UIGestureRecognizerDelegate,UITableViewDataSource, UITableViewDelegate,JHRecordViewDelegate, ChatInputViewDelegate {
  ...
  private var recordV = JHRecordView()
...
//                under the cell
  ...
  // MARK: lazy Method
    private lazy var recordImageView:UIImageView = {
        let  recordImageView = UIImageView(image: UIImage(named: "Slice 40"))
...
//        recordImageView.animationDuration = 0.8
  ...
  private lazy var tableView: UITableView = {
    ...
    tableView.backgroundColor = UIColor(hexString: "#FAFAF7")
...
//          constant: kNavigationHeight + KStatusBarHeight
      ...
      addRightAction(UIImage(named: "更多"), #selector(toSetting), self)
    }
  ...
  //    offset：value which from self.view.bottom to inputView.bottom
    private func layoutInputView(offset: CGFloat) {
      inputViewTopConstraint?.constant = -83 - offset
      tableViewBottomConstraint?.constant = -83 - offset
      UIView.animate(withDuration: 0.25, animations: {
        self.view.layoutIfNeeded()
      })
    }
    ...
    func beginRecord() {
        recordV.frame = UIScreen.main.bounds
        recordV.delegate = self
        UIApplication.shared.windows.first?.addSubview(recordV)
        recordV.showVoice()
        let dur = 60.0
    ...
    //松手
    func endPressGestureRecognizer(){
        recordV.finishRecord()
        NIMSDK.shared().mediaManager.stopRecord()
    }
//MARK:JHRecordViewDelegate
    func speekSoSort(){
        recordV.removeFromSuperview()
        showToast(localizable("说话时间太短"))
    }
    ...
    func limitTimedAutoSend(path: String) {
        viewmodel.sendAudioMessage(filePath: path) { error in
            if let e = error {
  ...
  //    MARK: ChatInputViewDelegate
    //长按说话
    public func longPressClickEvent(sender: UILongPressGestureRecognizer) {
        print("location:\(sender.location(in: sender.view))")
        switch sender.state {
        case .began:
            beginRecord()
          print("state:begin")
...
//    }
  ...
  //    play
    public func playAudio(_ filePath: String, didBeganWithError error: Error?) {
      ...
      if let e = error {
        ...
        recordV.playBtn.isSelected = false
        recordV.playBtn.imageView?.startAnimating()
      }
    }
  ...
  //    record
    ...
    public func recordAudioProgress(_ currentTime: TimeInterval) {
        recordV.starRecord(dur: currentTime)
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
      ...
      if menuView.textField.isFirstResponder {
        menuView.textField.resignFirstResponder()
          ...
          if menuView.currentType == .emoji{
              menuView.emojiView.isHidden = true
          }
          if menuView.currentType == .more{
              menuView.moreView.isHidden = true
          }
          if menuView.currentType == .audio{
              menuView.recordView.isHidden = true
          }
        layoutInputView(offset: 0)
      }
    }
